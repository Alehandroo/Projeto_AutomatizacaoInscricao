...
def process_csv(file_id):
    ...
    csv_data = response.decode('utf-8').splitlines()
    ...
    for row in csv_data:
        ...
        if len(parameters) != 8:
            print("O arquivo CSV deve conter exatamente 8 parâmetros.")
            continue

        # Atribuir os valores dos parâmetros a variáveis
        param1 = parameters[0]
        param2 = parameters[1]
        param3 = parameters[2]
        param4 = parameters[3]
        ...
        # Restante do código: Realizar operações com os parâmetros, etc.
        # ...
...
def exportar(aluno,lista):
    spreadsheet = client.open_by_key(workBook_exportar)
    worksheet1 = spreadsheet.worksheet(log_book)
...
# Aceder ao worksheet DataBase
client = gspread.authorize(creds)
sheet = client.open_by_key('1mEwN7mD1gxVVuiV8CdFOUYlsxlTPJrUCUqb4VbQgwGQ')
worksheet = sheet.worksheet('DataBase')

#Aceder ao log_book
workBook_exportar = '1lhp_olC3TTMrSiX34CH1ePlkJUBmQweZS4-aNJNcLgI'
log_book = 'log_book'
...
for linha in values[1:]:
    if not any(linha):
        continue  # Continuar se vir uma linha vazia até ao final
